...
function M.levelSelector(id, level, maxLevel, names, levelData, labelColor)
	...
	if level >= 1 and level <= maxLevel then
		...
		M.hint(string.format('Threshold: %.0f   Freio mín: %.0f', l[1], l[2]))
		M.hint(string.format('Intensidade: %.2f   Smooth: %.1f', l[3], l[4]))
		M.hint(string.format('Rear Bias: %.0f   Trail Brake: %.0f   Recovery: %.0f', l[5], l[6], l[8]))
		M.hint(string.format('Fator de Curva: %.0f', l[9]))
